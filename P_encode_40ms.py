...
# Open
file_time = '202001091942'
location = '250Collector' + '_' + file_time + '.txt'
...
for d in range(len(raw_data) - 1):
    ...
    for e in range(len(emg_set)):
        if len(emg_set[e]) > 50 and len(emg_set[e]) < 120:
...
for d in range(len(dataset)):
    if len(dataset[d]) == 14 and dataset[d][1] > 1000:
        full_idx.append(d)
...
# Save
sav_loc = 'encode_8ch_' + file_time + '.csv'
f = open(sav_loc, 'w')
for d in range(len(prop_data)):
    for v in range(len(prop_data[d])):
