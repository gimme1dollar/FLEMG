...
- P_data_collect.py : 센서로 부터 데이터를 받아와 트레이닝 시킬 데이터를 모으는 프로그램.
- P_infer_csv.py : csv파일 형식으로 된 데이터를 읽어서 LSTM 네트워크를 트레이닝 시키는 프로그램. 
- P_collect_train.py : 데이터를 사용자가 지정한 만큼 모은 다음에 네트워크를 트레이닝 시키는 프로그램. (P_data_collect.py랑 P_infer_csv.py를 순차적으로 진행하면 돼서 굳이 만들지 않음.)
...
- EMG Sensor는 VSPE로 가상 시리얼 포트 만든다음에 OpenBCI GUI 통해서 EMG데이터 보내는 방식
(1) 소프트웨어 환경 세팅
